...
export default class StickerCommand extends Command {
  ...
  static override async execute(ctx: IncomingCommand) {
    ...
    if (!isNaN(asId)) {
      const view = await renderItem(asId, 'sticker', ctx.message.author.id)
      if (!view) {
        await reply(ctx, 'Não encontrei um sticker com esse ID.')
        return
      }
    ...
    const results = await VanitiesDB.searchStoreItemsByType('sticker', query, 100)
    if (results.length === 0) {
      await reply(ctx, 'Não encontrei um sticker com esse nome.')
      return
    }
    if (results.length === 1) {
      const view = await renderItem(results[0]!.id, 'sticker', ctx.message.author.id)
